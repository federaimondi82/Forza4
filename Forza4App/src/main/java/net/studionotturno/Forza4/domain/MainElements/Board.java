...
public class Board {
	...
	/**
	 * Metodo per conoscere tutte le colonne libere disponibili in fase di gioco
	 * La struttura dati indica l'eventuale indice e l'indice della colonna libera
	 * @return le colonne libere disponibili in fase di gioco
	 */
	public Map<Integer,Integer> getColonneLibere() {

		//scorre la riga più in alto
		//controlla se ci sono token e riempie una lista con i fori delle colonne senza token
		List<Hole> list=getRowsList(5).stream().filter(row->{
			return row.isEmpty();
	 ...
	 * di conseguenza diminuiscono i fori liberi della colonna
	 * @param col  la colonna dove il giocatore vuole inserire il gettone
	 */
	public void addToken(ArrayList<Hole> col) {

		Predicate<Entry<Integer,ArrayList<Hole>>> pred=(colonna->{
			return colonna.getValue().equals(col);
		});
